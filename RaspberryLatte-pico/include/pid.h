...
/**
 * \brief Clear the internal parameters of the discrete_integral d and set the lower and upper bounds.
 * \param d Pointer to discrete_integral stuct that will be initalized.
 * \param lower_bound Pointer to a floating point lower bound on the integral's value. If NULL, set to -10000.
...
/**
 * \brief Compute the area under the curve since the last time this function was called and add to i's sum field.
 * This area is computed assuming a linear change between the two data points: sum += 0.5*(prev_val + cur_val)*(cur_t - prev_t).
 * If there was no previous datapoint, then p is saved internally and 0 is returned. 
 * 
 * \param i Pointer to discrete_integral stuct that will be added to.
 * \param p Datapoint to add to discrete_integral
 * 
 * \returns The first time the function is called on i after initalizing or clearing it, 0 is returned.
 * After the, the value of integral after adding the datapoint p is returned.
 */
